...
def deletar(conexao, sql):
    ...
    # o finally é a finalização de tudo. Independentimente do try e except o finally
    finally:
...
vsql = "UPDATE tb_contatos SET T_NOMECONTATO='Bruno', T_EMAILCONTATO='bruno@gmail.com' WHERE N_IDCONTATO=2"

# atualizar(vconexao, vsql)
...
def consulta(conexao, sql):
    ...
    c = conexao.cursor()
    c.execute(sql)
    # Para fazer uma consulta de todos registros
    resultado = c.fetchall()
    return resultado


vsql = "SELECT * FROM tb_contatos"
res = consulta(vconexao, vsql)
